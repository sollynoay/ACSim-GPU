...
@wp.kernel
def soft_ac_generate_gaussian_accumulate_slice(
    ...
    u = (dval - lowlimit) / resolution
    kc = wp.int32(wp.floor(u))

    denom = float(0.0)
    k = kc - radius
    while k <= kc + radius:
        if k >= 0 and k < length:
            diff = (u - float(k)) / sigma_bins
            wk = wp.exp(-0.5 * diff * diff)
            denom = denom + wk
        k = k + 1
    ...
    while k <= kc + radius:
        if k >= 0 and k < length:
            diff = (u - float(k)) / sigma_bins
            wk = wp.exp(-0.5 * diff * diff) / denom
            wp.atomic_add(out_sum_image, k, x, wk * ival)
        k = k + 1
...
def process_buffers_to_sum_image(
    ...
    # --------------------------------------------------------
    # sum_image accumulation
    # Corresponds to:
    #   for i in reversed(range(depth)): use ray_path_wrt
    #   for i in reversed(range(depth-1)): use ray_path_prt
    # --------------------------------------------------------
    ...
    for i in range(depth-1 , -1, -1):
        wp.launch(
            kernel=soft_ac_generate_gaussian_accumulate_slice,
            dim=(h, w),
            inputs=[
                ray_path_wrt_wp,
                buf_c_wp,
            ...
            outputs=[sum_image_wp],
            device=device,
        )
    
    # PRT part: i = depth-1 ... 0 in old code was depth-1 down to 0 for range(depth-1, -1, -1)
    # because old code used range(depth-1, -1, -1) after WRT loop over depth+1.
    # Here depth is already D, so second loop matches original semantics with D-2 ... 0.
...
# Quick smoke test
# ============================================================
def vis_single_channel(x, title="", log=False):
        ...
        plt.imshow(y, cmap="viridis" if not log else "magma")
        plt.title(title)
        plt.axis("off")
...
def main():
    ...
    sum_image_log_u8 = np.clip(sum_image_log / 100.0 * 255.0, 0, 255).astype(np.uint8)
